...
public class WeeklyClientPasswordService : IWeeklyClientPasswordService
{
    ...
    {
        var managementIds = (await _context.WeeklyPasswordSubscribers
            .Select(s => s.AdminId).ToListAsync()).ToHashSet();
        var clientIds = (await _context.WeeklyClientPasswordSubscribers
            .Select(s => s.AdminId).ToListAsync()).ToHashSet();
        ...
        return admins.Select(a => new EligibleAdminDto
        {
            ...
            Email = a.Email,
            Role = a.UserType.ToString(),
            IsSubscribedToManagement = managementIds.Contains(a.Id),
            IsSubscribedToClient = clientIds.Contains(a.Id)
        }).ToList();
    }
